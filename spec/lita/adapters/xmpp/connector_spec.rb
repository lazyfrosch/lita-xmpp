...
describe Lita::Adapters::XMPP::Connector, lita: true do
  ...
  it "sets the JID properly when a node and domain are supplied" do
    subject = described_class.new(robot, "user@example.com", "secret", 'jabber.example.com')
    expect(subject.jid).to eq("user@example.com/bot")
  end

  it "sets the JID properly when a resource is supplied" do
    subject = described_class.new(robot, "user@example.com/wrong", "secret", 'jabber.example.com')
    expect(subject.jid).to eq("user@example.com/bot")
  end
  ...
  describe "#connect" do
    ...
    let(:callback) { instance_double("Lita::Adapters::XMPP::Callback") }
    ...
    before do
      ...
      allow(Lita::Adapters::XMPP::Callback).to receive(:new).and_return(callback)
    ...
    it "connects to XMPP" do
      expect(subject.client).to receive(:connect).with('jabber.example.com')
      subject.connect
    end

    it "connects to XMPP Server" do
      subject = described_class.new(robot, "user", "secret", 'jabber.example.com')
    ...
    it "registers a message callback" do
      expect(Lita::Adapters::XMPP::Callback).to receive(:new).with(
        robot,
        roster
      ).and_return(callback)
  ...
  describe "#join" do
    ...
    it "stores a SimpleMUCClient for the room" do
      subject.join('conference.jabber.example.com', "foo")
      expect(subject.mucs['foo@conference.jabber.example.com']).to eq(muc)
    end
    ...
    it "doesn't attempt to join a room it's already in" do
      ...
      2.times { subject.join('conference.jabber.example.com', "foo") }
    end
  end
  ...
  describe "#join rooms" do
    ...
    it "joins each room" do
      rooms.each { |room| expect(subject).to receive(:join).with('conference.jabber.example.com', room) }
      subject.join_rooms('conference.jabber.example.com', rooms)
    end
  end
  ...
  describe "#list_rooms" do
    ...
    it "returns an array of room JIDs for the MUC domain" do
      allow(browser).to receive(:muc_rooms).with('conference.jabber.example.com').and_return(
        "123_456@conference.jabber.example.com" => "Room 1",
        "789_012@conference.jabber.example.com" => "Room 2"
      )
      expect(subject.list_rooms('conference.jabber.example.com')).to eq([
        "123_456@conference.jabber.example.com",
        "789_012@conference.jabber.example.com"
      ])
    end
  end
  ...
  describe "#part" do
    ...
    it "parts from a room" do
      subject.mucs["#foo@conference.jabber.example.com"] = muc
      expect(muc).to receive(:exit)
      subject.part('conference.jabber.example.com', "#foo")
    end
